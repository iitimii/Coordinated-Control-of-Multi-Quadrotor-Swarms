...
class MRACControl(BaseControl):
    """MRAC Controller class for Crazyflies."""

    def __init__(self, drone_model: DroneModel, g: float = 9.8):
        super().__init__(drone_model=drone_model, g=g)
        if self.DRONE_MODEL not in [DroneModel.CF2X, DroneModel.CF2P, DroneModel.RACE]:
            print("[ERROR] MRACControl requires DroneModel.CF2X or DroneModel.CF2P or DroneModel.RACE")
            exit()
    ...
    def _compute_K(self, psi=0):
        ...
        Ac = np.hstack((a_sub, np.zeros((6, 6)))) # Part of A that is controllable

        
        b_sub = np.array([[1/m, 0, 0, 0],
                          [0, 1/Ixx, 0, 0],
                          [0, 0, 1/Iyy, 0],
                          [0, 0, 0, 1/Izz]]) # need to change to adapt to X config
        ...
        # desired_poles = np.array([-2, -2, -2, -2, -2, -2, -3, -3, -3, -4, -4, -4])
        K = ct.place(A, B, desired_poles)
        # self.Kr_lin_ctr = np.linalg.solve(B, A - B @ K) 
        self.Kr_lin_ctr = np.linalg.pinv(B) @ (A - B @ K)

        self.Am = A - B@K
        self.Bm = np.copy(B)
        self.P = solve_lyapunov(self.Am.T, -Q)

        self.Gamma_x = np.eye(12) * 5e-3 # 0.005
        self.Gamma_r = np.eye(4) * 5e-3

        Kx = -K.T
        Kr = np.eye(4) # -inv(C*inv(A-BKx)*B)
