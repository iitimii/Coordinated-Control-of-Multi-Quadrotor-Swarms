...
def run(
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        target_xyzs = v_formation(obs, side_length=1, center_point=(0, 0, 1))
    ...
    env.close()
    logger.save()
    logger.save_as_csv("v_formation")
    if plot:
        logger.plot()

def v_formation(current_states, angle, side_length=1.0, center_point=np.array([0, 0, 1])):
