...
from controllers.rl_controller import RLControl
...
def run(
    ...
    if drone in [DroneModel.CF2X, DroneModel.CF2P, DroneModel.RACE]:
        ctrl = [RLControl(drone_model=drone) for i in range(num_drones)]
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        for j in range(num_drones):
            action[j,:] = ctrl[j].computeControlFromState(control_timestep=env.CTRL_TIMESTEP,
                                                                state=obs[j],
